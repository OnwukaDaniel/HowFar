...
class DirectReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        chattingRef = FirebaseDatabase.getInstance().reference.child(CHAT_REFERENCE).child(myAuth).child(otherParticipant(lastMsg.participants))
            ...
            timeRef.setValue(ServerValue.TIMESTAMP).addOnSuccessListener {
                timeRef.get().addOnSuccessListener { snapshot ->
                    if (snapshot.exists()) {
                        ...
                        myChat.timesent = rawTime
                        lastMsg.read = true
                        lastMsg.timeseen = rawTime
                        chattingRef.child(myChat.uniqueQuerableTime).setValue(myChat).addOnSuccessListener {
                            chattingRef.child(lastMsg.uniqueQuerableTime).setValue(lastMsg)
                            receiverChattingRef.child(myChat.uniqueQuerableTime).setValue(myChat)
                            val notificationManager = context!!.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
                            //
                            sendNewNotification(myChat, context, notificationId)
                        }
                    }
                }
            }
