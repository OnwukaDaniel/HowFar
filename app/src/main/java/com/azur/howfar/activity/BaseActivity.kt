...
abstract class BaseActivity : AppCompatActivity() {
    ...
    var runnable = object : Runnable {
        override fun run() {
            ...
            val ref = FirebaseDatabase.getInstance().reference.child(ONLINE_PRESENCE).child(myAuth)
            timeRef.setValue(ServerValue.TIMESTAMP).addOnSuccessListener {
                timeRef.get().addOnSuccessListener {
                    if (it.exists()) {
                        onlineTime = it.value.toString()
                        val onlinePresenceData = OnlinePresenceData(uid = myAuth, lastTimePosted = onlineTime)
                        ref.setValue(onlinePresenceData).addOnSuccessListener {}
                    }
                }
            }
