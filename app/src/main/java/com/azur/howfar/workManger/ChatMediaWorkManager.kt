        ...
        timeRef.setValue(ServerValue.TIMESTAMP).addOnSuccessListener {
            timeRef.get().addOnSuccessListener { timeSnapshot ->
                if (timeSnapshot.exists()) {
                    ...
                    var mediaUploadTask = mediaRef.putFile(
                        when (chatData.messagetype) {
                            MessageType.AUDIO -> Uri.parse(chatData.audioData.storageLink)
                            MessageType.VIDEO -> Uri.parse(chatData.videoData.storageLink)
                            else -> Uri.EMPTY
                        }
                    )
                    if (chatData.messagetype == MessageType.PHOTO) {
