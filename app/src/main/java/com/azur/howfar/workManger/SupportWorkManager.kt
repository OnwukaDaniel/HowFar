...
class SupportWorkManager(private val context: Context, val params: WorkerParameters) : Worker(context, params) {
    ...
    private fun upload() {
        val mySupportRef = FirebaseDatabase.getInstance().reference.child(CONTACT_SUPPORT).child(user!!.uid).child(data.uniqueQuerableTime)
    ...
    private fun sendFinal(data: ChatData) {
        ...
        val myProfileRef = FirebaseDatabase.getInstance().reference
            .child(CHAT_REFERENCE)
            .child(user!!.uid)
            .child(CONTACT_SUPPORT)
            .child(data.uniqueQuerableTime)
        myProfileRef.setValue(data) // SAVES IT TO THE CHAT
    }
