...
import com.azur.howfar.models.CallAnswerType.NO_RESPONSE
...
class IncomingCallDialog : AppCompatActivity(), View.OnClickListener {
    ...
    private fun initCall() {
        ...
        val callFromConstant = intent.getIntExtra("callFrom", 0)
    ...
    private fun requestAudioFocus() {
        val audioFocusRequest = AudioFocusRequest.Builder(AudioManager.AUDIOFOCUS_GAIN).setOnAudioFocusChangeListener {
    ...
    private fun setupLocalVideo() = runOnUiThread {
        ...
        agoraEngine!!.setupLocalVideo(VideoCanvas(localSurfaceView, VideoCanvas.RENDER_MODE_FIT, uid))
    }
                ...
                timeRef.setValue(ServerValue.TIMESTAMP).addOnSuccessListener {
                    timeRef.get().addOnSuccessListener { time ->
                        if (time.exists()) {
                            ...
                            val myCallerHistory = FirebaseDatabase.getInstance().reference.child(CALL_HISTORY).child(myAuth)
