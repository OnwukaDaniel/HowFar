...
class ChatActivity2 : BaseActivity(), View.OnClickListener, QuoteHelper, MiscHelper {
            ...
            override fun run() {
                ...
                onlineRef.get().addOnSuccessListener {
                    if (it.exists()) {
                        ...
                        binding.userLastSeen.text = if (timeDiff < 60_000) "Online" else Util.formatSmartDateTime(localTime.toString())
                    }
                }.addOnFailureListener {
                    binding.userLastSeen.text = ""
                }
    ...
    override fun onResume() {
        ...
        val currentWall = pref.getInt(getString(R.string.chatWallpaper) + receiverUid, defaultWall)
        singleChatBubbleColor = pref.getString(getString(R.string.chatBubbleColor) + receiverUid, "#660099")!!
        window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS)
        window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS)
        Glide.with(this).load(currentWall).centerCrop().into(binding.chatBackground)
                            ...
                            if (snapshot.exists()) {
                                ...
                                chattingRef.child(myChat.uniqueQuerableTime).setValue(myChat).addOnSuccessListener {
                                    ...
                                    blockedRef.get().addOnSuccessListener { blocked ->
                                        ...
                                        if (blocked.exists()) {
                                            for (i in blocked.children) blockedList.add(i.value.toString())
                                            if (myAuth !in blockedList) receiverChattingRef.child(myChat.uniqueQuerableTime).setValue(myChat)
                                        } else receiverChattingRef.child(myChat.uniqueQuerableTime).setValue(myChat)
                                    }
                                }
                            }
                        }
...
class ImageDialogFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        imageDialogViewModel.imageName.observe(viewLifecycleOwner) {
            //val msg = "Send"
...
class ChatAdapter2 : RecyclerView.Adapter<RecyclerView.ViewHolder>(), ItemTouchHelperAdapter {
    ...
    private fun scrollOnClickQuotedText(datum: ChatData) {
        val handler = Handler(Looper.getMainLooper())
        var scrollPosition = dataset.size
    ...
    private fun isVisible(@Nullable rootView: View = rv, @Nullable view: View?, minPercentageViewed: Int): Boolean {
        val mClipRect = Rect()
