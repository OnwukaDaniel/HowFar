...
class FragmentDisplayVideo : Fragment(), View.OnClickListener, SeekBar.OnSeekBarChangeListener, CustomExoplayerMethods {
    ...
    private fun tryLoadingLocalFirst() {
        if (chatData.videoData.storageLink.contains("https")){
            loadFile(Uri.parse(chatData.videoData.storageLink))
            return
        }
        ...
        if (!realFile.exists()) lockFile.createNewFile()

        if (lockFile.exists()) {
            file = Uri.fromFile(realFile)
            downloadVideo(file)
        } else loadFile(Uri.fromFile(realFile))
    }
    ...
    override fun onClick(p0: View?) {
        when (p0?.id) {
            ...
            R.id.play_download -> downloadVideo(file)
        }
    }
    ...
    private fun downloadVideo(file: Uri) {
        ...
        val request = Request(videoUrl, file)
